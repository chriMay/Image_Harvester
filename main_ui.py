...
class HarvesterInterface:
    ...
    def __init__(self, devMgr) -> None:
        ...
        self.list_devices()
        self.devices_om = OptionMenu(self.input_frame, self.clicked_device, *self.devices_list)
        self.devices_om.grid(row=0, column=0)

        # Button to refresh the dropdown menu
        self.refresh_button = Button(self.input_frame, text="Aktualisieren", command=self.refresh_devices)
        ...
        self.path_label = Label(self.input_frame, text=self.saving_path.get(), relief="sunken")
        self.path_button = Button(self.input_frame, text="Speicherort wählen", command=self.select_path)
        ...
        self.settings_button = Button(self.input_frame, text="Einstellungen", command=self.device_settings_menu)
        self.settings_button.grid(row=2, column=0, sticky="we")

        # Button to make a test image
        self.test_button = Button(self.input_frame, text="Testbild", command=self.image_test)
        self.test_button.grid(row=3, column=0, sticky="we")

        # Buttons to start/stop the image harvesting
        self.start_button = Button(self.input_frame, text="Start", command=self.start_harvesting)
        self.stop_button = Button(self.input_frame, text="Stop", command=self.stop_harvesting)
        ...
        self.frameRate_info_label = Label(self.information_frame, text=f"Bildfrequenz: {self.device_settings.frameRate} Hz")
        self.exposureTime_info_label = Label(self.information_frame, text=f"Belichtungszeit: {self.device_settings.exposureTime} \u03BCs")
        self.gain_info_label = Label(self.information_frame, text=f"Gain(ISO): {self.device_settings.gain}")
    ...
    def refresh_devices(self):
        ...
        for device in self.devices_list:
            menu.add_command(label=device, command=lambda value=device: self.clicked_device.set(value))
    ...
    def list_devices(self):
        ...
        for i in range(self.devMgr.deviceCount()):
            ...
            self.devices_list.append(f"[{i}] {pDev.serial.read()} {pDev.product.read()}")
    ...
    def device_settings_menu(self):
        ...
        self.apply_button = Button(self.top, text="Übernehmen", command=self.apply_settings)
    ...
    def apply_settings(self):
        ...
        self.frameRate_info_label.configure(text=f"Bildfrequenz: {self.device_settings.frameRate} Hz")
        self.exposureTime_info_label.configure(text=f"Belichtungszeit: {self.device_settings.exposureTime} \u03BCs")
    ...
    def image_test(self):
        ...
        devHand = deviceHandler(self.devMgr, index, self.device_settings, self.saving_path.get())
    ...
    def start_harvesting(self):
        ...
        if self.devices_list[0][1] == "0": #Check if devices are found 
            ...
            index = int(self.clicked_device.get()[1])
            self.devHand = deviceHandler(self.devMgr, index, self.device_settings, self.saving_path.get())
