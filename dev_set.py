...
class Configuration:
    def __init__(self):
        ...
        # (x, y) (Pixels) starting from left upper corner of the original image
        self.snippet_position = (
            int(config.get("image", "snippet_position_x")),
            int(config.get("image", "snippet_position_y")),
        )

        # Size (Pixels) of the snippet in vertical (x-direction) and horizontal (y-direction) direction
        self.snippet_size = (
            int(config.get("image", "snippet_size_x")),
            int(config.get("image", "snippet_size_y")),
        )

        self.snippet_color = config.get("image", "snippet_color")
